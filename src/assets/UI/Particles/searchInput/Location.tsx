...
const Location: React.FC<location> = ({ style }) => {
    ...
    return (
        <>
            <div id="" className={` ${style ? "bg-[white]" : "bg-[#2BBEF9]"}  dropdown ${style ? `w-[100%]` : `w-[40%]`} h-[100%] ${style ? "rounded-lg" : "rounded-full"}  flex items-center justify-center gap-4 px-5 ${style ? "py-[10px]" : "py-0"}`}>
            ...
            {isShow ? <div id='scrollBar' className={`${style ? "w-[100%]" : "w-[37%]"} overflow-y-scroll absolute top-14 left-1 bg-white flex flex-col  h-60 rounded-xl`}>
